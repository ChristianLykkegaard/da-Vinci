...
\begin{flalign}
\hat{x}(k+1) = \Gamma \hat{x}(k) + \Phi K_d \hat{x}(k) + L_d (C\hat{x}(k)-y(k)) + M_d x_\text{ref}
\label{eq:observer}
\end{flalign}
\vspace{-0.6cm}
\begin{longtable}{p{.8\textwidth} p{.1\textwidth} p{.1\textwidth}} 
...
$\Gamma$ is the discretized system matrix \\
$\Phi$ is the discretized input matrix \\
$K_d$ is control gain calculated from the discretized matrices \\
$L_d$ is observer gain calculated from the discretized matrices \\
$M$ is gain correction for the observer \\
$N$ is gain correction for the system  
\end{longtable}
\vspace*{-0.2cm}
...
&y = Cx
\end{flalign*}
The equations are implement in simulink as shown in ???
