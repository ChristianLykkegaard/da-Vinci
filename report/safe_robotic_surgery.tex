...
Where  & & \\
$u(x) \in \mathbb{R}^{1 \times 1} $ {\color{red}{TJEK OP!!}} is a control signal where safety is ensured  & [$\cdot$] \\
$\tilde{u}(x) \in \mathbb{R}^{1 \times 1}$ {\color{red}{TJEK OP!!}} is a control signal to the linear state space system such that $\tilde{u}=\bar{N}\cdot x_\text{ref}-Kx $ & [$\cdot$] \\ 
$k_0(x) \in \mathbb{R}^{1 \times n}$ {\color{red}{TJEK OP!!}}is a control law that guarantees safety & [$\cdot$] \\ 
$\sigma(x) \in \mathbb{R}^{1 \times 1}$ {\color{red}{TJEK OP!!}} is a parameter that founds a linear combination between the two control inputs & [$\cdot$] \\ 
$K \in \mathbb{R}^{1 \times n}$ {\color{red}{TJEK OP!!}} is a constant feedback matrix where $n$ is the number of states & [$\cdot$] \\
$\bar{N} \in \mathbb{R}^{1 \times 1}$ {\color{red}{TJEK OP!!}} is a constant to ensure unity gain from reference to output & [$\cdot$] \\
$x \in \mathbb{R}^{n \times 1}$ {\color{red}{TJEK OP!!}}is the state vector& [m] 
\end{longtable}
\vspace*{-0.2cm}
...
\begin{figure}[H]
	...
	\caption{Block diagram of the control system for slide position. MATLAB implementation is found in \autoref{app:slide_implement_1}. \color{red}{A,B,C matricerne skal skrives generelt i stedet for 1D tilf\ae ldet.}}
