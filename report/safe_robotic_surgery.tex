...
\begin{defn}[Control Barrier Function]
...
$L_fB(x)$ & is the Lie derivative of $B(x)$ along the vector field  $f(x)$, i.e. $\frac{d B(x)}{d x}f(x)$ & [$\cdot$] \\ 
$L_gB(x)$ & is the Lie derivative of $B(x)$ along the vector field  $g(x)$, i.e. $\frac{d B(x)}{d x}g(x)$ & [$\cdot$] 
\end{tabular}
\vspace*{0.2cm}
\end{defn}
...
u(x) =
\begin{cases}
	\tilde{u}(x) \kk &\text{if} \mm x \in \,\, \text{\gls{Yfancy}}\subset\mathcal{X}_0 \\
	 k_0(x)  \kk &\text{if} \mm x \in \,\, \text{\gls{Tfancy}}= \mathcal{X}_0\setminus\mathcal{Y}
...
\begin{flalign}
u(x,\tilde{u}) &= \sigma(x)k_0(x)+(1-\sigma(x))\tilde{u}(x) \label{eq:control_law}
...
Note the two extremities of  $\sigma(x)$:
\begin{flalign*}
\sigma(x) = 
\begin{cases}
0 \mm &\Rightarrow \mm \text{Pure control by pole placement, i.e. $u(x) = \tilde{u}(x) =  \bar{N} x_\text{ref}-Kx$ } \\
...
\end{flalign}
Where $\kappa$ is a design variable. High values of $\kappa$ implies increased aggressiveness. \Autoref{eq:control_law_safety} indeed ensures safety for the closed loop system $\dot{x} = f(x)+g(x)k_0(x)$. This is easily proven as:
\begin{flalign*}
