...
A closed loop control system, $\Gamma_\text{cl} = (f_\text{cl},h,X,X_0.X_u,D)$, is unsafe if there exist a time $t \in [0,T]$ such that the trajectory $\phi_{X_0}^{\bar{d}} : [0,T]$ satisfy: 
	...
	\end{flalign*}
The closed loop system $\Gamma_\text{cl}$ is safe if there are no unsafe trajectories. Here \gls{X} is the set of all allowed states. \gls{phi} is the set t of all allowed initial conditions with bounded disturbance $\bar{d}$
