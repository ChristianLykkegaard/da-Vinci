...
\end{figure}
It is seen that the Lie derivatives from \autoref{fig:meas_lie_2} are quite different that from \autoref{fig:lie2}. This might be due to an imprecise state estimation.
...
The results from this chapter suggest some improvement in the current setup:
\begin{itemize}
\item Replace the TCP/IP communication channel with a \gls{udp} channel such that the topic \texttt{joint\_states} updates faster, i.e. preferable with 2\,kHz. This will obviously improve the precision and resolution and ultimately save human lifes.
...
\item Improve the bump function $\sigma(x)$ for the second order system such that the braking distance is optimized.
\end{itemize}
The above considerations concludes this chapter.