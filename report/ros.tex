...
The overall code structure should look like the tree structure found below:

\vspace{0.5cm}

\Tree [.\texttt{<name of workspace>}  
 \scriptsize  \color{green}{\texttt{CMakeLists.txt}}\\\text{(auto-generated)}
  [.\scriptsize  \texttt{build/} \text{all make-files}\\\text{(auto-generated}) ] 
 [.\scriptsize  \texttt{devel/} \text{setup files,}\\\text{libraries etc.} ]  
  [.\scriptsize  \texttt{src/}  \text{\hspace{0.5cm}all packages are placed here, i.e.:}\\\texttt{\hspace{0.5cm}davinci\_description}\\\texttt{\hspace{0.5cm}davinci\_driver}\\\texttt{\hspace{0.5cm}davinci\_moveit\_config} ] 
  ]
  
\vspace{0.5cm}

The package where most of the development takes place is in the \texttt{davinci\_moveit\_config} package located in the \texttt{src} folder. The structure of this is therefore shown in further details below.
...
[.\scriptsize \texttt{davinci\_}\\\scriptsize \texttt{moveit\_config/} \scriptsize  \color{green}{\texttt{CMakeLists.txt}} [.\scriptsize  \texttt{src} \scriptsize \texttt{MoveGroupInterface.cpp}\\\text{(main C++ interface)} ]  \scriptsize  \texttt{config} \scriptsize  \texttt{launch} \scriptsize  \color{green}{\texttt{package.xml}} ]   
