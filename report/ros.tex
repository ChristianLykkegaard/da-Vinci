...
\end{tabular}
\end{table}
\item \textbf{Planning Groups:} It is from here possible to describe the joints of the \texttt{p4\_arm} of da-Vinci. The Orocos \gls{kdl} kinematic solver seems to be dependent of at least six \gls{dof} (six active joints). It  
\item \textbf{Robot Poses:}
\item \textbf{End Effectors:}
\item \textbf{Passive Joints:}
\item \textbf{Configuration Files:}
\end{enumerate}
\begin{figure}[H]
	\includegraphics[scale=0.45]{setup_assistant}
